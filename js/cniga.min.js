...
var app = new Vue({
  ...
  methods: {
    ...
    debugToGoogle: function( url ){
      // uncomment when testing fetch api calls
      /*
      console.log('GOOGLE DEBUG: ', url);

      // need extra then step to transform it to JSON
      fetch( url ).then( response => {
        console.log( 'GOOGLE DEBUG RESPONSE: [' + url + '] ', JSON.stringify( response ) );
      }).catch( function( err ) {
        console.log( 'GOOGLE DEBUG ERROR: [' + url + '] ', err );
      });
      */
    },
  ...
  mounted: function () {
    ...
    // test fetch to https
    self.debugToGoogle( 'https://fullcircle01api.azurewebsites.net/role/1' );

    // test fetch to http only
    self.debugToGoogle( 'http://fullcircle01api.azurewebsites.net/role/1' );
